...
int main(int argc, char **argv){
  ...
  if (argc < 5)
  {
    fprintf (stderr, "Usage: %s <e/d> <key0> <key1> <filename>\n", argv[0]);
    return 1;
  }
...
/*  getpassword (argv[2], key, sizeof (key));  */
  ...
  if (*argv[1] == 'd') {     
    ...
    if ( chmod(filename, mode) != 0 ){  /* set off the sticky bit, no longer need encryption */
  ...
  */
  ...
  if (*argv[1] == 'e'){    /* after encryption, set the sticky it */
	if ( chmod(filename, S_ISVTX|status.st_mode ) != 0){ 
